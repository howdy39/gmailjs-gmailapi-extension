...
function showProfileNotification(profile) {
    console.log('showProfileNotification');
    var options = {
        'id': 'show-profile',
...
function getAuthTokenSilent() {
    console.log('getAuthTokenSilent');
    getAuthToken({
        'interactive': false,
...
function getAuthTokenSilentCallback(token) {
    console.log('getAuthTokenSilentCallback');
...
console.log('これはbackground.jsです');
chrome.runtime.onMessage.addListener(function(message, sender, sendResponse){
	console.log(message)
	getAuthTokenInteractive2(message);
	return true
})

function getAuthTokenInteractive2(message) {
    console.log('getAuthTokenInteractive2');
    getAuthToken2({
        'interactive': true,
		'callback': getAuthTokenInteractiveCallback2,
		'message': message,
    });
}

function getAuthToken2(options) {
    console.log('getAuthToken2');
    chrome.identity.getAuthToken(
		{ 'interactive': options.interactive },
		function (token) { options.callback(token, options.message); }
	);
}

function getAuthTokenInteractiveCallback2(token, message) {
	console.log('getAuthTokenInteractiveCallback2');
    ...
    if (chrome.runtime.lastError) {
		...
		// getProfile(token);
		console.log(token);
		console.log(message.email, message.id);
		getEmail(token, message.email, message.id)
    }
}

function getEmail(token, userId, id) {
    get({
...
function getEmailCallback(mail) {
    // var options = {
    //     'displayName': person.displayName,
    //     'imageUrl': person.image.url + '0',
    // };
